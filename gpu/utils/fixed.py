...
class Shape(hdl.ShapeCastable):
    ...
    def __init__(self, shape, f_bits=0):
        ...
        if self.i_bits < 0 or self.f_bits < 0:
            raise TypeError(
                f"fixed.Shape may not be created with negative bit widths (i_bits={self.i_bits}, f_bits={self.f_bits})"
            )
...
class Value(hdl.ValueCastable):
    ...
    def reshape(self, f_bits):
        ...
        if f_bits > self.f_bits:
            result = Shape(shape, f_bits)(
                hdl.Cat(hdl.Const(0, f_bits - self.f_bits), self.as_value())
            )
    ...
    def clamp(self, lo, hi):
        ...
        return Value(self.shape(), Mux(self > hi, hi, Mux(self < lo, lo, self)))

    def saturate(self, shape):
        if not isinstance(shape, Shape):
            raise TypeError(
                f"Cannot `saturate` to bounds of {shape!r} as it is not a fixed.Shape"
            )
        if not shape.i_bits <= self.i_bits:
            raise ValueError(
                f"Cannot `saturate`: shape.i_bits={shape.i_bits} > self.i_bits={self.i_bits} would have no effect."
            )
    ...
    def _binary_op(
        ...
        rhs,
        operator,
        callable_f_bits=lambda a, b: max(a, b),
        pre_reshape=True,
        post_cast=True,
    ):
...
class Const(Value):
    ...
    def _max_value(self):
        return (
            2 ** (self._shape.i_bits + self._shape.f_bits - (1 if self.signed else 0))
            - 1
        )

    def _min_value(self):
        if self._shape.signed:
            return -1 * 2 ** (self._shape.i_bits + self._shape.f_bits - 1)
        else:
            return 0
