...
def flatten_signals(signals) -> Iterable[Value]:
    """
...
def shape_of(value) -> Shape | ShapeCastable:
...
def const_of(value: int, shape) -> Any:
...
def binary_tree_reduce(
    *values, neutral: Value, operator: Callable[[Value, Value], Value]
) -> Value:
    ...
    while len(min_layers) > 1:
        ...
        min_layers = [
            operator(a, b) for a, b in zip(min_layers[::2], min_layers[1::2])
        ] + tail
...
def and_value(*values):
    return binary_tree_reduce(*values, neutral=C(-1), operator=operator.and_)


def generic_min_value(*values, operator: Callable[[Value, Value], Value]) -> Value:
...
def min_value(*values) -> Value:
    return generic_min_value(*values, operator=operator.lt)


def max_value(*values) -> Value:
