...
`timescale 1 ps / 1 ps
module soc_system_fb #(
		parameter BITS_PER_SYMBOL              = 8,
		parameter NUMBER_OF_COLOR_PLANES       = 4,
		...
		parameter DROP_INVALID_FIELDS          = 1,
		...
		parameter USER_PACKETS_MAX_STORAGE     = 0,
		...
		output wire [5:0]   mem_master_rd_burstcount,    //                  .burstcount
		...
		output wire         control_interrupt_irq,       // control_interrupt.irq
		input  wire [3:0]   control_address,             //           control.address
	...
	wire  [63:0] rd_ctrl_vob_data_data;                 // rd_ctrl:vob_data_data -> video_out:av_st_din_data
	...
	wire  [63:0] pkt_trans_rd_av_st_dout_data;          // pkt_trans_rd:av_st_dout_data -> rd_ctrl:pkt_rd_data_data
	...
	alt_vip_packet_transfer #(
		...
		.MAX_PACKET_SIZE_READ              (307200),
		.PREFETCH_THRESHOLD_READ           (0),
		.MAX_CONTEXT_NUMBER_READ           (1),
		.CONTEXT_BUFFER_RATIO_READ         (2),
		.BURST_TARGET_READ                 (32),
	...
	alt_vip_vfb_sync_ctrl #(
		...
		.CONTEXT_WIDTH            (8),
		.TASK_WIDTH               (8),
		.FRAME_BUFFER_LENGTH      (1228800),
		.ANC_BUFFER_LENGTH        (512),
		.MAX_WIDTH                (640),
		.MAX_HEIGHT               (480),
		.NUM_BUFFERS              (3),
		...
		.reader_control_address       (control_address),                 //           reader_control.address
		.reader_control_byteenable    (control_byteenable),              //                         .byteenable
		.reader_control_write         (control_write),                   //                         .write
		.reader_control_writedata     (control_writedata),               //                         .writedata
		.reader_control_read          (control_read),                    //                         .read
		.reader_control_readdata      (control_readdata),                //                         .readdata
		.reader_control_readdatavalid (control_readdatavalid),           //                         .readdatavalid
		.reader_control_waitrequest   (control_waitrequest),             //                         .waitrequest
		.reader_control_irq           (control_interrupt_irq)            // reader_control_interrupt.irq
	);
