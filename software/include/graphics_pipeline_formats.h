 ...
 *
 * This header defines all enums, structs, and types used throughout the
 * PixelForge GPU pipeline. It mirrors the Amaranth Python definitions for
...
/* ============================================================================
 * Basic Types & Constants (aligned with gpu/utils/types.py and
 * gpu/utils/layouts.py)
 * ============================================================================
 */

/* Fixed-point formats
 * - FixedPoint_mem:   SQ(16,16) -> 32-bit signed storage (used for
 * matrices/attributes in memory)
 */
 ...
 * info is a union in RTL; use the larger constant_value payload (4x
 * FixedPoint_mem) to cover both modes. When mode == CONSTANT, fill
 * constant_value[4]. When mode == PER_VERTEX, fill per_vertex.address/stride
 * and ignore constant_value.
 */
typedef struct {
...
typedef struct {
  ...
  pixelforge_fixed16_16_t texture_transform[16];
