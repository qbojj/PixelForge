/*
 * PixelForge Demo: Rotating Object with Directional Diffuse Lighting
 *
 * This demo showcases:
...
static void configure_gpu(pixelforge_dev *dev, uint32_t idx_addr, uint32_t idx_count,
    ...
    /* Normal matrix (upper 3x3 of model-view, as that is affine) */
    float nm[9];
    ...
    mat.ambient[0] = fp16_16(0.2f);
    mat.ambient[1] = fp16_16(0.2f);
    ...
    /* Directional light (position.w=0 for directional) */
    pixelforge_light_t light = {0};
    light.position[0] = fp16_16(0.5f);
    light.position[1] = fp16_16(1.0f);
    light.position[2] = fp16_16(0.3f);
    light.position[3] = fp16_16(0.0f); /* directional */
    light.ambient[0] = fp16_16(0.2f);
    light.ambient[1] = fp16_16(0.2f);
    light.ambient[2] = fp16_16(0.3f);
    light.diffuse[0] = fp16_16(1.0f);
    light.diffuse[1] = fp16_16(1.0f);
    light.diffuse[2] = fp16_16(0.9f);
...
int main(int argc, char **argv) {
    ...
    printf("PixelForge Lighting Demo: Rotating Cube with Directional Diffuse Light\n");
    printf("Rendering %d frames...\n", frames);

    /* Create geometry */
    struct vram_block vb_block;
    if (vram_alloc(&dev->vram, VB_REGION_SIZE, 64, &vb_block) != 0) {
        fprintf(stderr, "Failed to allocate VRAM for vertex buffer\n");
    ...
    for (int frame = 0; frame < frames && keep_running; frame++) {
        ...
        /* Build model-view matrix (rotate on all axes + translate back) - same as demo_cube */
        ...
        configure_gpu(dev, idx_addr, idx_count, pos_addr, norm_addr, col_addr, stride,
                     buffer_phys, mv, p);
