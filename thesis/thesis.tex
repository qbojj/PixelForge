...
\[
C_{final} = C_{\textit{src}} \cdot f_{\textit{src}} \; \text{op} \; C_{\textit{dst}} \cdot f_{\textit{dst}}
\]
gdzie:
\begin{itemize}
...
\[
C_{final} = C_{\textit{src}} \cdot \alpha_{\textit{src}} + C_{\textit{dst}} \cdot (1 - \alpha_{\textit{src}})
\]
...
Ze względu na~charakter platformy FPGA oraz~zgodność ze~specyfikacją OpenGL~ES~1.1 Common-Lite, projekt wykorzystuje arytmetykę stałoprzecinkową~\cite{fixed-point} zamiast zmiennoprzecinkowej.
...
\begin{itemize}
  \item transformację pozycji do~przestrzeni widoku: $\mathbf{p}_{\textit{view}} = M_{mv} \cdot \mathbf{p}_{\textit{model}}$,
  \item transformację pozycji do~przestrzeni obcinania: $\mathbf{p}_{\textit{clip}} = M_p \cdot \mathbf{p}_{\textit{view}}$,
  \item transformację normalnej do~przestrzeni widoku: $\mathbf{n}_{\textit{view}} = M_{norm} \cdot \mathbf{n}_{\textit{model}}$, gdzie $M_{norm} = (M_{mv}^{-1})^T$.
\end{itemize}
...
\begin{enumerate}
  ...
  Każdy wierzchołek otrzymuje swój zestaw współrzędnych ekranowych $(x_{\textit{screen}}, y_{\textit{screen}})$.
...
Aplikacje budowane są~przy~użyciu cross-compilera ARM (np. \code{arm-linux-gnueabihf-gcc}) i~uruchamiane na~płycie DE1-SoC.
