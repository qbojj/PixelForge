...
Dla każdego fragmentu o~współrzędnej głębokości $z$ porównywana jest~wartość z~aktualną wartością w~buforze głębokości $z_{buffer}$. Fragment przechodzi test, jeśli:
\[
z \;  \text{op} \; z_{buffer}
\]
...
\begin{itemize}
  ...
  \item \textbf{demo\_alpha} --- obracający się~wachlarz przezroczystych kwadrantów z~alfa-blendingiem (SRC\_ALPHA / ONE\_MINUS\_SRC\_ALPHA) oraz jeden oświetlający (additive-blending - SRC\_ALPHA / ONE), demonstrując operacje blendingu kolorów.
